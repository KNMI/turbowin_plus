...
{
   // called from: Move_log_fiLE() [main.java]
      ...
      protected Void doInBackground() throws Exception
      {
         ...
         if (Desktop.isDesktopSupported())
         {
            ...
            try
            {
               ...
               http_adres = "http://esurfmar.meteo.fr/cgi-bin/meteo/display_vos_ext.cgi?callchx=";
               ...
               if (e.getButton() == MouseEvent.BUTTON1)           // left mouse button
               {
                  ...
                  else if (RS232_connection_mode == 5 || RS232_connection_mode == 6)       // Mintaka Star USB or Mintaka Star WiFi
                  {
                     ...
                     try
                     {
                        ...
                        boolean local_tray_icon_clicked = true;
                        boolean StarX = false;
                        main_RS232_RS422.RS232_Mintaka_Star_And_StarX_Read_Sensor_Data_PPPP_For_Obs(local_tray_icon_clicked, StarX); // retrieve mybarometer.pressure_reading
                     } // try
                     catch (NumberFormatException en)
                     {
                        info = "no sensor data available";   // eg just after start-up (takes 1 minute to start collecting) 
                  ...
                  else if ( (RS232_connection_mode == 3 || RS232_connection_mode == 9) && (defaultPort != null) )    // AWS (serial)
                  {
                     // check if we receive valid data (parameter day is arbitrary, could eg also be month etc.)
                     try
                     {
...
/***********************************************************************************************/   
  ...
  /***********************************************************************************************/   
   ...
   private void display_IMMT_on_leaflet_map(List<String> immt_list)
   {
      ...
      if ( (Desktop.isDesktopSupported()) && ((main.logs_dir != null) && (main.logs_dir.compareTo("") != 0)))
      {
         ...
         try
         {
            ...
            String full_path_leaflet_maps_html_file = main.logs_dir + java.io.File.separator + LEAFLET_MAPS_HTML_FILE;
            Obsen_In_leaflet_Map(immt_list, full_path_leaflet_maps_html_file);
            ...
            { 
               // if file is null  
               System.out.println("+++ unable to create dynamic html file for leaflet Maps plot [function: display_IMMT_on_leaflet_map()] (" + ex1 + ")");
            }

         } // try
         catch (MalformedURLException ex) 
         { 
            System.out.println("+++ unable to create dynamic html file for leaflet Maps plot [function: display_IMMT_on_leaflet_map()] (" + ex + ")");
         }
         catch (IOException ioe) 
         { 
            System.out.println("+++ unable to create dynamic html file for leaflet Maps plot [function: display_IMMT_on_leaflet_map()] (" + ioe + ")");
         }
   ...
   private void Obsen_In_leaflet_Map(List<String> immt_list, String full_path_leaflet_maps_html_file)
   {
      ...
      // NB this function is still in background thread
      //
      // NB see also: Input_Position_menu_actionPerformed(0 [main.java]
      // NB see also: set_latest_obs_values(0 [DASHBOARD_latest_obs.java]
      ...
      try
      {
         ...
         out.write(LEAFLET_CSS_URL);out.newLine();
         out.write(LEAFLET_CSS_INTEGRITY);out.newLine();
         out.write("  crossorigin=\"\"/>");out.newLine();
         out.write(LEAFLET_JS_URL);out.newLine();
         out.write(LEAFLET_JS_INTEGRITY);out.newLine();
         out.write("  crossorigin=\"\"></script>");out.newLine();
         out.newLine();
         out.write("  <!-- Load Esri Leaflet from CDN -->");out.newLine();
         out.write(LEAFLET_ESRI_URL);out.newLine();
         out.write(LEAFLET_ESRI_INTEGRITY);out.newLine();
         out.write("  crossorigin=\"\"></script>");out.newLine();
         out.newLine();
         ...
         for (String obs : immt_list) 
         {
            ...
            if (obs.length() > 80)
            {
               ...
               if ( (true_wind_dir.trim().length() > 0) && (true_wind_speed.trim().length() > 0) )
               {
                  ...
                  infowindow_wind = "wind: " + true_wind_dir_hulp + "&#176" + " / " + true_wind_speed + " " + wind_units;
               ...
               // MSL air pressure
               //
               String infowindow_msl_pressure = "MSLP: ";
               if (pressure_MSL.trim().length() > 0)           // there is something present
               {
                  try
                  {
                     ...
                     {
                        infowindow_msl_pressure = "MSLP: " + pressure_MSL.substring(0, 3) + "." + pressure_MSL.substring(3, pressure_MSL.length()) + " hPa";
                     }
                     else
                     {
                        infowindow_msl_pressure = "MSLP: " + "1" + pressure_MSL.substring(0, 3) + "." + pressure_MSL.substring(3, pressure_MSL.length()) + " hPa";
                     }
                  }
                  catch (NumberFormatException e)
                  {
                     infowindow_msl_pressure = "MSLP: ";
                  }
               ...
               // pop-up text
               //
               String infowindow_total = "\"" + "<h3>" + local_call_sign.trim() + " weather observation" + "</h3>"
                              + "<p>"
               ...
               out.write("  L.marker([" + lat_marker + "," + lon_marker + "]).addTo(map).bindPopup(" + infowindow_total + ").openPopup();");out.newLine();
            ...
            protected List<String> doInBackground() throws Exception
            {
               ...
               if (immt_file.exists() && immt_file.length() > 0)     // length() in bytes
               {
                  ...
                  try
                  {
                     ...
                     while ((record = in.readLine()) != null)
                     {
                        immt_list.add(teller++, record);
                     }
                     in.close();
                     
                     // display the map via a web browser
                     display_IMMT_on_leaflet_map(immt_list);
                  }
                  catch (IOException ex) { }
               ...
               else
               {
                  ...
                  JOptionPane.showMessageDialog(null, info, main.APPLICATION_NAME + " error", JOptionPane.WARNING_MESSAGE);
                  immt_list = Collections.emptyList(); //                // Collections.emptyList() returns an immutable list, i.e., a list to which you cannot add elements if you want to perform any operation on your list, then create new instance of list and return it.
               } // else
...
/*                                                                                             */
/***********************************************************************************************/
private void log_memory_statistics()
{
...
/***********************************************************************************************/
   ...
   public static final String APPLICATION_VERSION           = "3.1.0 (build 30-July-2018)"; // NB DO NOT FORGET TO COMPILE ABOUT.JAVA AFTER A CHANGE OF THIS STRING
